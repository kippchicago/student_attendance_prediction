...
# model
mu <- int + cum_enroll * b_e + cum_absent * b_a + school_int[school_id] + cum_enroll * school_slope_e[school_id] + cum_absent * school_slope_a[school_id]
...
```{r checking training data against predictions}
ggplot() +
  geom_point(data = yca_filtered_bayes %>% 
               rowid_to_column("n"), aes(x = year_end, y = n))
...
all_pred_draws <- rbind(pred_new_data$`11`, pred_new_data$`12`, pred_new_data$`13`, pred_new_data$`14`)
...
```{r individual student example varying slope and int}
pred_student_vsi_ex <- new_data[1976,] %>% 
  mutate(predicted_yr_end = new_pred_values[1976],
         sd_plus = predicted_yr_end + sd(all_pred_draws[,1976]),
         sd_minus = predicted_yr_end - sd(all_pred_draws[,1976]),
         dec_ada = 1 - (cum_absent/cum_enroll),
         month = "Jun")

```

```{r initial graphs for student example}
pred_student_vsi <- pred_student_vsi_ex %>% 
  ...
  mutate(sd_minus = if_else(grepl("dec", key), value, sd_minus),
         ...
         sd_minus = round(sd_minus*100),
         sd_plus = round(sd_plus*100),
         value = round(value * 100))

pred_student_vsi %>% 
  mutate(value = if_else(!grepl("dec", key), NA_real_, value)) %>% 
  ggplot(aes(x = month, y = value)) +
  geom_point() +
  scale_y_continuous(breaks = c(84, 88, 92, 96),
                     limits = c(82, 96),
                     name = "Average Daily Attendance (%)") +
  ggrepel::geom_text_repel(label = "Dec ADA") +
...
scale_x_discrete(limits = c(5, 11),
                 labels = c("Dec", "Jun"),
                 name = "") +
  scale_y_continuous(name = "Average Daily Attendance (%)") +
scale_color_manual(values = kipp_palettes$kipp_div) +
theme_kipp_light() +
theme(legend.position = "none")
```

```{r greta calculate for new school level predictions}
# school_level_preds <- pred_student_vsi %>% 
#   filter(grepl("dec", key))
# 
# school_no <- c(1:6)
# 
# school_no %>% 
#   map(~draws_coef["int"] + draws_coef["b_e"] * school_level_preds$cum_enroll + draws_coef["b_a"] * school_level_preds$cum_absent + draws_coef[sprintf("school_int[%s,1]", .x)] + draws_coef[sprintf("school_slope_e[%s,1]", .x)] * school_level_preds$cum_enroll + draws_coef[sprintf("school_slope_a[%s,1]", .x)] * school_level_preds$cum_absent)

school_level_preds_new <- bind_rows(school_level_preds,
                                    school_level_preds,
                                    school_level_preds,
                                    school_level_preds,
                                    school_level_preds,
                                    school_level_preds) %>% 
  mutate(school_id = c(1:6),
         month = "Jun") %>% 
  select(-c(sd_plus:sd_minus),
         -c(key:value))

school_pred_cum_enroll <- as_data(school_level_preds_new$cum_enroll)
school_pred_cum_absent <- as_data(school_level_preds_new$cum_absent)
school_pred_school_id <- as_data(school_level_preds_new$school_id)

school_pred_mu <- int + school_pred_cum_enroll * b_e + school_pred_cum_absent * b_a + school_int[school_pred_school_id] + school_pred_cum_enroll * school_slope_e[school_pred_school_id] + school_pred_cum_absent * school_slope_a[school_pred_school_id]

school_pred_new_data <- greta::calculate(school_pred_mu, draws)

school_new_pred_values <- colMeans(rbind(school_pred_new_data$`11`, school_pred_new_data$`12`, school_pred_new_data$`13`, school_pred_new_data$`14`))
```


```{r graphs school level predictions}
school_new_all_draws <- rbind(school_pred_new_data$`11`, school_pred_new_data$`12`, school_pred_new_data$`13`, school_pred_new_data$`14`)

school_num <- c(1:6)

pred_sd <- school_num %>% 
  map(~sd(school_new_all_draws[,.x])) %>% 
  unlist()

plot_school_level <- school_level_preds_new %>% 
  mutate(predicted_values = school_new_pred_values,
         dec_value = school_level_preds$value,
         sd = pred_sd,
         sd_minus = round((predicted_values - sd) *100,2),
         sd_plus = round((predicted_values + sd) * 100,2),
         predicted_values = round(predicted_values*100, 2),
         ) %>% 
  tidyr::gather(key, value, -c(school_id:month, sd:sd_plus)) %>% 
  mutate(month = if_else(grepl("dec", key), "Dec", month),
         month = factor(month, levels = c("Aug",
                                          "Sep",
                                          "Oct",
...
plot_school_level %>% 
  mutate(school_id = as.character(school_id),
         school_name = c("KAMS", "KAP", "KAC", "KBCP", "KOA", "KOP", rep("", 6))) %>% 
  ggplot(aes(x = month, y = value)) +
  geom_path(aes(group= school_id, color = school_id)) +
  geom_point(aes(color = school_id)) +
  ggrepel::geom_text_repel(aes(label = school_name),force = 1, nudge_x = .0625 ) +
  scale_y_continuous(breaks = c(84, 88, 92, 96),
                     limits = c(82, 96),
                     name = "Average Daily Attendance (%)") +
  scale_x_discrete(name = "") +
  scale_color_manual(values = kipp_palettes$kipp_qual) +
  theme_kipp_light() +
  theme(legend.position = "none")
...
```{r graph sd school level predictions}
plot_school_level %>%
  mutate(month = as.integer(month),
         sd_minus = if_else(grepl("dec", key), 84, sd_minus),
         sd_plus = if_else(grepl("dec", key), 84, sd_plus),
         school_id = as.character(school_id),
         school_name = c("KAMS", "KAP", "KAC", "KBCP", "KOA", "KOP", 
                         "KAMS", "KAP", "KAC", "KBCP", "KOA", "KOP"),
         actual = if_else(month == 5, NA_real_, 90)) %>% 
  ggplot(aes(x = month, y = value)) +
  geom_path(aes(group= school_id, color = school_id)) + 
  geom_point(aes(color = school_id)) +
  geom_point(aes(x = month, y = actual), color = "hotpink") +
  geom_ribbon(aes(ymin = sd_minus, ymax = sd_plus, group = school_id), color = "gray", alpha = .25, lty = 2) +
...
```{r checking new data predictions vs actual}
ggplot() +
  geom_point(data = new_data %>% 
               rowid_to_column("n"), aes(x = year_end, y = n))
